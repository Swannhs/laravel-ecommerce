...
if (! function_exists('platform_path')) {
    function platform_path(?string $path = null): string
    {
        ...
        if (str_starts_with($normalized, 'core')) {
            $normalized = preg_replace('/^core/i', 'Core', $normalized);
        } elseif (str_starts_with($normalized, 'packages')) {
            $normalized = preg_replace('/^packages/i', 'Packages', $normalized);
        } elseif (str_starts_with($normalized, 'plugins')) {
            $normalized = preg_replace('/^plugins/i', 'Plugins', $normalized);
        } elseif (str_starts_with($normalized, 'themes')) {
            $normalized = preg_replace('/^themes/i', 'Themes', $normalized);
        }
