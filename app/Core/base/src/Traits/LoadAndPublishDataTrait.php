...
trait LoadAndPublishDataTrait
{
    ...
    protected function getPath(?string $path = null): string
    {
        ...
        if (! Str::contains($modulePath, app_path('Plugins'))) {
            $namespacePath = str_replace('\\', '/', $this->getDashedNamespace());

            if (str_starts_with($namespacePath, 'core')) {
                $namespacePath = preg_replace('/^core/i', 'Core', $namespacePath);
            } elseif (str_starts_with($namespacePath, 'packages')) {
                $namespacePath = preg_replace('/^packages/i', 'Packages', $namespacePath);
            } elseif (str_starts_with($namespacePath, 'plugins')) {
                $namespacePath = preg_replace('/^plugins/i', 'Plugins', $namespacePath);
            } elseif (str_starts_with($namespacePath, 'themes')) {
                $namespacePath = preg_replace('/^themes/i', 'Themes', $namespacePath);
            }

            $modulePath = app_path($namespacePath);
        }
