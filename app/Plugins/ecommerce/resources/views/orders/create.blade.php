...
@section('content')
    <create-order
        ...
        :payment-methods="{{ json_encode(is_plugin_active('payment') ? \App\Plugins\Payment\Enums\PaymentMethodEnum::labels() : []) }}"
        :payment-statuses="{{ json_encode(is_plugin_active('payment') ? \App\Plugins\Payment\Enums\PaymentStatusEnum::labels() : []) }}"
    ></create-order>
@endsection
